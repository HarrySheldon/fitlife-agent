...
- [x] Replace product-facing literals with translation keys while preserving structured record content and Agent output. Evidence: the complete frontend suite passes 51 tests, including localized Evaluation summaries/labels, exact Agent Markdown preservation, and the product favicon regression.
...
- [x] Run frontend tests and production build. Evidence: Vitest passes 9 files/51 tests; Vite transforms 2438 modules and completes the production build.
- [x] Inspect desktop/mobile `/settings/general` and one core page in both languages. Evidence: Chrome DevTools inspection covered `/settings/general` and `/` in `en-US` and `zh-CN` at 1440x1000 and 390x844. All eight scenarios had the expected `<html lang>`, translated headings, `scrollWidth === innerWidth`, no horizontal overflow, and no JavaScript or console errors.
