...
from backend.schemas import AccountPasswordChangeRequest, AuthSession, AuthenticatedUser
...
def change_password(
    ...
    return ok(
        session.model_dump(),
        message_for_request("ACCOUNT_PASSWORD_CHANGED", request, user),
    )
...
def revoke_other_sessions(
    ...
    repository = FileIdentityRepository(get_settings().data_dir)
    session = RevokeOtherSessions(repository, _issue_session).execute(user)
    return ok(
        session.model_dump(),
        message_for_request("ACCOUNT_SESSIONS_REVOKED", request, user),
    )
